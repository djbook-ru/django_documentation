...
class basic_tokenize(tokenize):
    ...
    def next(self):
        ...
        while True:
            ...
            while offset < len(text) and (text[offset].isspace() or text[offset] == '(' or text[offset] == '`'):
                offset += 1
            sPos = offset
            # Find end of word
            while offset < len(text) and not (text[offset].isspace() or text[offset] == '(' or text[offset] == '`'):
                offset += 1
            ePos = offset
            self._offset = offset
